...
void main() {
  ...
  ToiletRecord urine(String id, DateTime at) => ToiletRecord(
    toiletId: id,
    petId: 'pet-1',
    recordedAt: at,
  ...
  test(
    'buckets multiple records on the same day into one count, ignoring time-of-day',
    () {
      final records = [
  ...
  test('an optional type filter restricts counting to that ToiletType', () {
    ...
    final urineOnly = aggregator.aggregateDailyCounts(
      records,
      type: ToiletType.urine,
    );
    final stoolOnly = aggregator.aggregateDailyCounts(
      records,
      type: ToiletType.stool,
    );
