...
> 2026-08-16 時点の次回作業メモ。現在地は ../SESSION_HANDOVER.md
...
- [ ] 🔴 **輸出コンプライアンスの回答** ← **保留中。調査フェーズを設ける**
      → `docs/BACKLOG.md` の「輸出コンプライアンス」を読むこと。
      Firebase が独自のTLS実装を同梱している点が論点で、即答できない。
      **書類が必要と判断した場合、取得に数週間かかる。** 実登録の直前に
      着手すると 9/30 に間に合わない。
