...
**聞かれる順番どおり**に書きます。上から順に答えてください。

| # | 画面に出る文言 | 入れるもの |
|---|---|---|
| 1 | キーストアのパスワードを入力してください | 任意のパスワード。**必ず記録** |
| 2 | 新規パスワードを再入力してください | **1と同じものをもう一度** |
| 3 | 姓名は何ですか | お名前、または `wanote` |
| 4 | 組織単位名は何ですか | 空欄のままEnter |
| 5 | 組織名は何ですか | 空欄のままEnter |
| 6 | 都市名または地域名は何ですか | 空欄のままEnter |
| 7 | 都道府県名または州名は何ですか | 空欄のままEnter |
| 8 | 2文字の国コードは何ですか | `JP` |
| 9 | ...でよろしいですか | **`はい`** と入力してEnter |

> **2番は「同じ質問が2回出た」のではなく、打ち間違い防止の確認です。**
> パスワードは画面に表示されないため、keytool は必ず2回聞きます。
> 1回目と違うものを入れると「パスワードが一致しません」と出てやり直しになります。

> **9番は日本語環境では `はい` だけが通ります。**
> `yes` も `y` も**受け付けられません**（この環境で実際に試して確認しました）。
> 通らなかった入力はすべて「いいえ」扱いになり、**3番から聞き直し**になります。
> 何度か繰り返すと「再試行が多すぎます」でコマンドごと終了します。
>
> - 画面には `[いいえ]:` と既定値が出ています。**そのままEnterも「いいえ」**です
> - 3番に戻ってしまったら、慌てず 3〜8 をもう一度答えて、9番で `はい` と入れてください
> - すでに何度か失敗して終了している場合、鍵はまだ作られていません。
>   0-1 のコマンドから実行し直してください

> 環境によっては、最後にもう1つ聞かれることがあります。
>
> ```
...
> ```
>
> ここは**そのままEnter**で構いません。0-2 の `keyPassword` には
> 1番のパスワードを書きます。
