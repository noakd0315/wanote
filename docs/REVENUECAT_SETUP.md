...
> ⚠️ **サブスクリプションと違い、自動保存されません。**
> A-1 は入力すると自動で保存されますが、**App内課金は自分で保存**する必要が
> あります（2026-08-20 に実画面で確認）。同じ画面の作りに見えて挙動が違うので、
> 入力しただけで離れると消えます。

1. 左メニュー **収益化** → **App内課金**
2. 「＋」→ **消耗型** を選択
   - ⚠️ **必ず「消耗型」**。「非消耗型」にすると1回しか買えなくなります
   - > **製品IDは再利用できません。** 間違えると別IDで作り直しになり、
     >  アプリ側の設定変更まで波及します
   - 参照名: `AIチケット5回` / **製品ID: `ai_tickets_5`**
3. 同様に `AIチケット15回` / **製品ID: `ai_tickets_15`**
4. 価格: `ai_tickets_5` = 日本 **¥300** / 米国 **$2.99**、
   `ai_tickets_15` = 日本 **¥800** / 米国 **$7.99**
5. **ローカリゼーション**（表示名30文字・説明45文字の上限）

   **Play の上限（タイトル全角27・説明全角100）より厳しいので、
   この文字列は両ストアで共用できます。**

   | | 日本語 | English |
   |---|---|---|
   | `ai_tickets_5` 表示名 | `AI相談チケット 5回分` | `5 AI Consultation Tickets` |
   | `ai_tickets_5` 説明 | `AI相談を5回追加。有効期限はありません` | `Adds 5 AI consultations. Never expires.` |
   | `ai_tickets_15` 表示名 | `AI相談チケット 15回分` | `15 AI Consultation Tickets` |
   | `ai_tickets_15` 説明 | `AI相談を15回追加。有効期限はありません` | `Adds 15 AI consultations. Never expires.` |

   > 「有効期限はありません」は事実です。`tickets_remaining` は単純な加算
   > カウンタで、期限のフィールドが存在しません。
   >
   > なお**無料枠がチケットより先に消費される**ので（`ai_usage_repository.dart`）、
   > 「無料が残っているのに買ったぶんが減る」ことは起きません。45文字には
   > 入らないため、プラン画面やヘルプで説明します。
6. **保存**（自動保存されません）
...
1. **アプリに同等の仕組みが既にある。** キャンペーンコードの引き換えが
   実装済みで（プラン画面に入力欄あり）、RevenueCat のプロモーション特典
   付与APIで premium を1か月分付与します。**iOS と Android の両方で動きます**
   （Apple のオファーコードは iOS 専用）
2. **公開前は使えない。** サブスクリプションが審査を通っていないと発行できません
3. **プロモーション画像（1024×1024）が必須化する。** オファーコードや支払い
   シートの表示には画像の提供が要求されます

公開後のマーケティング（プレス配布、離脱ユーザーの呼び戻し）で必要になったら
そのとき設定します。
...
| 無料テスターなのに広告が出ない | そのアカウントに Promotional が残っている（下記） |

## ダッシュボードから手で付与した無料期間は「期限なし」になる

付与ダイアログの duration は既定が **Lifetime** です。そのまま付与すると、
Customer ページに開始日だけが並び、期限が出ません。アプリから見ると終わらない
プレミアムなので、広告は永久に出ず、AIも無制限のままになります（2026-08-21、
実機テストで発生）。

**招待コード経由の付与は必ず `monthly`（1か月）です。** コードから叩く経路も、
紹介報酬を後追いで渡す経路も、同じ定数を送っています。期限なしになっていたら、
それは手で付与したものです。

取り消しは Customers → App User ID（Firebase の uid）で検索 → Entitlements の
その行の **Revoke**。取り消したあとはアプリを1分以上バックグラウンドに置いて
戻せば反映されます（復帰時にエンタイトルメントを読み直すため、再起動は不要）。

無料プランの挙動を確かめたいときは、付与していない別アカウントで試すのが
確実です。
