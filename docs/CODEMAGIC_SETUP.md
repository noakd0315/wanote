...
| 名前 | 内容 | 必要なもの |
|---|---|---|
| **`verify`** | analyze / test / **Android ビルド** | **なし。今日から使えます** |
...
> `verify` は gitignore されている設定ファイルの**ダミーを CI 内で生成**
> します（`firebase_options.dart` と `google-services.json`）。
> 前者は実行すると例外を投げるだけ、後者は形だけのプレースホルダなので、
> **誤ってどこかに繋がることはありません。**

> **✅ この手順はローカルで検証済みです**（2026-08-14）。
> GitHub から**まっさらにクローンし、CI と同じ手順を実行**して
> analyze・テスト321件・Androidビルドが通ることを確認しました。
> **初回ビルドが赤くなることはない**はずです。
...
**Android ビルドは `verify` に追加済みです**（2026-08-14）。
これが今日の不正XMLを捕まえる唯一のステップでした。
