...
最終更新: 2026-08-22 深夜（**実機テストの指摘を修正して push・デプロイ済み**。
設定周り（有料App契約・お支払いプロファイル・配信地域）を進める。
金額表示は Sandbox の既知の制約と判明しクローズ）

**再開時にまず読むもの: このファイルの「2026-08-23 に最初にやること」**
...
**今日（8/22）の予定だった「設定周り」が丸ごと残っている。** 実機テストで
出た指摘の修正に時間を使ったため。下の「設定周りの残り」から着手する。

## 設定周りの残り（PM 作業）

| # | 作業 | 状態 |
|---|---|---|
| B-1a | **Apple 有料App契約** | 税務フォーム入力済み。**「有効」になったか確認** |
| B-1b | **Google お支払いプロファイル** | 🔴 **未着手。Apple と同じ情報が使える** |
| — | **Apple Small Business Program**（30%→15%） | 未加入なら要申請。B-1a と同じ画面 |
| — | **配信地域を日本・米国に絞る** | 両ストア。`docs/BACKLOG.md` に詳細 |
| — | **iOS ビルドを手動起動** | Codemagic の `iOS build to TestFlight`。push 済みの修正が未反映 |

**AdMob の本番IDはクローズドテストの後。** 未公開アプリでは本番広告が配信
されないため、テスト期間中はテストIDのままの方がテスターに広告が出る。
忘れないよう下の「公開前の関門」に記載してある。

## ✅ 解決: 金額が USD で出る（2026-08-23 クローズ）

**アプリの問題ではなかった。** Sandbox / TestFlight では、商品情報の取得が
テスターのストアフロントに関係なく **USD を返すことがある**（Apple の既知の
制約。RevenueCat・Apple フォーラムに同じ報告多数）。本番では起きない。

**見分け方**: ペイウォールが `$` でも、**購入シート（Apple の確認ポップアップ）は
正しい通貨で出る**。購入シートは実際のアカウントのストアフロントで解決される
ため。2026-08-23、iPhone の購入シートが **￥** であることを確認してクローズ。

ストアフロントと通貨コードはログに出る（`BillingRepository`）。Android なら
`adb logcat` で読める。画面に出す診断パネルは revert 済み（`2b4c1df`）だが、
**TestFlight に配信済みのビルドには残っている** — 次の iOS ビルドで消える。

## 2026-08-21〜22 に入れた修正（push 済み、Worker デプロイ済み）

| コミット | 内容 |
|---|---|
| `26e2cb8` | 期限切れなのに広告が出ず、買ったチケットが見えない |
| `65b30bf` | チケット加算を取引IDで冪等にし、取りこぼしを拾う |
| `72510a6` | 残回数の表示条件、レポートの通院歴削除、生体認証の間合い |
| `2b37a13` | 健康レポートに健康カテゴリタグを渡す |
| `505f813` `96aab97` | ドキュメント2件 |

**Android 実機にインストール済み**（`354961112985499`、上書き）。
**iOS は未反映** — Codemagic の iOS ワークフローは手動起動のみ。

### 確認してほしいこと（次に触ったとき）

- チケット購入直後にバッジの数字が動くか
- 使い放題（有料プラン・キャンペーン適用中）はバッジが出ないか
- 顔認証が少し間をおいてから始まるか（700ms）
- **初回起動でチケットが一度だけ増える。** 冪等化より前の購入には払い出し
  済みの記録が無いため。**二度目以降も増えるなら想定外**

## 2026-08-22 に分かったこと

**ダッシュボードから手で付与した無料期間は期限なし（Lifetime）になる。**
アプリから見ると終わらないプレミアムなので、広告は永久に出ず、AIも無制限の
まま。**「バグで広告が出ない」の顔をして現れる。** コード経由の付与は必ず
`monthly` なので、期限が無ければ手動付与。取り消しは Customers → uid →
Entitlements の Revoke。詳細は `docs/REVENUECAT_SETUP.md`。
...
現在のブランチ: **`main`**（`72510a6` まで push 済み）

## 🔴 公開前の関門（忘れると収益ゼロ）

**AdMob の本番広告ユニットID。** いまは Google のテストIDで、
**そのまま公開すると広告は出るのに収益が1円も発生しない。**

必要なもの（PM が AdMob 管理画面で作成、6つ）

```
アプリID Android / iOS          ca-app-pub-XXXX~YYYY   ※ ~ 区切り
インタースティシャル Android / iOS  ca-app-pub-XXXX/ZZZZ   ※ / 区切り
バナー Android / iOS              ca-app-pub-XXXX/ZZZZ
```

**アプリIDだけは AndroidManifest.xml と Info.plist に直書き**が要る
（ビルド時変数では渡せない）。残り4つは `config/prod.json`。
