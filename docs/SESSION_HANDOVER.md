...
設定周り（有料App契約・お支払いプロファイル・配信地域）は丸ごと翌日へ。
両ストアとも金額表示が未解決）
...
## 🔴 未解決: 両ストアとも金額表示がおかしい（2026-08-22 PM報告）

**アプリ側の修正はすべて入れた上で残っている。** 概ね良好、ただし金額表示だけ
残る、という報告。

| | 症状 |
|---|---|
| Android | 常に円表示 |
| iOS | Sandbox を日本にしてあるのに USD 表示 |

金額の文字列は RevenueCat の `StoreProduct.priceString` をそのまま出している
（アプリ側で組み立てていない）ため、**ストア側の地域設定か、テスト
アカウントの地域**を見る筋が濃い。

- Android: Play Console で日本・米国しか価格を設定していない。
  ライセンステスターのアカウントの国と、Play ストアアプリの国設定を見る
- iOS: App Store Connect の価格・提供地域と、Sandbox テスターの国を見る
