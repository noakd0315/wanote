...
最終更新: 2026-08-23（**設定周りを一日で片付けた**。両ストアとも手数料 15%、
iOS の設定漏れ4件を修正。Google の税務情報は審査中。
残るのはスクリーンショットの撮り直しと審査用スクリーンショット）

**再開時にまず読むもの: このファイルの「2026-08-24 に最初にやること」**

**文書の全体像は `docs/INDEX.md`。** 残作業は `docs/REMAINING_WORK.md`、
後回しにした判断は `docs/BACKLOG.md`。

**このファイルは `C:\Dev\wanote\docs\` に移しました**（2026-08-22）。
旧 `C:\Dev\docs` は `C:\Dev\docs_superseded_20260822` へ退避済み。
...
**8/23 は設定周りを一日で片付けた。** ブラウザ連携（Claude in Chrome）で
両ストアのコンソールを直接見て回り、**設定漏れを5件見つけて直した**。

## 🔴 次回いちばん先にやること（2026-08-23 夜に確定）

### 1. RevenueCat の Transfer behavior を決める（PM作業・設定変更）

**1つのストアアカウントで、いくつでも wanote アカウントをプレミアムにできる。**
アプリ側では塞げない。`BACKLOG.md` の「1つのストアアカウントで、いくつでも
プレミアムにできる」に選択肢と推奨を書いた。**Keep with the original App User ID**
が PM の期待（復元は登録した wanote アカウントのみ）に合う。

### 2. 修正版で再確認する

対象データを削除済みのため、**この日の指摘は再現確認ができていない。**
`1.0.0+7` を入れてから確認する。

| # | 指摘 | 直した内容 |
|---|---|---|
| 1 | 新規アカウントにチケット20枚 | 過去分の突き合わせをやめた。**自分がおこなった購入だけ**を報告する |
| 2 | 無意味な質問に英語で返る | 言語は飼い主の設定で決まると明示。**Worker デプロイ済み**（2026-08-23、`d360c252`） |
| 3 | 復元ボタンが見えない | AppBar から一覧下部へ。何が復元されるかの説明を追加 |
| 4 | アカウント削除中に登録画面 | 削除前にペットの購読を止め、待ち画面を出す |
| 5 | 給餌量のAI相談ボタンが左寄り | 中央寄せに戻した |
| 6 | AI画面でローディングが不揃い | 全画面の待ちを共通化し、3画面とも同じに |

**あわせて確認**: 手で付与した期限なし（Lifetime）の `premium` が残っていないか。
残っていると、ストア側で何をしても永久にプレミアムのままになる。

### 3. バグ調査（2026-08-23 夜に PM 報告・未着手）

| # | 症状 |
|---|---|
| A | **招待コードもサブスクも無いのに、AI相談に無料回数もチケット回数も出ない**（デグレード） |
| B | **チケットを買っただけなのにプレミアム扱いになる**（当日）。本来はチケットが増え、その分が広告の対象外になるだけ |

## 🔍 この2件は同じ原因の可能性が高い

**どちらも「`hasUnlimitedSubscription` が true になっている」で説明がつく。**

- A: バッジは使い放題のあいだ何も出さない（`ai_usage_badge.dart` の `_label`
  冒頭）。無料回数が出ないのは、アプリが使い放題だと思っているから
- B: チケット購入は `_onCustomerInfoUpdate` を通る。そこで `premium` の権利を
  読み、状態が変われば `setUnlimitedSubscription(uid, true)` が走る。
  **チケットがプレミアムにしたのではなく、もともと有効だった権利が
  そこで初めて反映された**、という筋

## 🔴 疑うべきは、ストアのアカウント側に残っている `premium`

wanote のアカウントに招待コードもサブスクも無くても、**端末の Google / Apple
アカウントが権利を持っていれば、顧客情報を読んだ時点で今の wanote アカウントに
反映される。** 心当たりが2つある。

1. **手で付与した期限なし（Lifetime）の `premium`** ── 2026-08-22 に同じ現象で
   調べた件。Customers → uid → Entitlements で、期限が出ない `premium` が
   無いか見る
2. **テスト購入したサブスクが、ストアのアカウントに残っている**

## 調べ方

```
RevenueCat → Customers → uid で検索 → Entitlements
```

ここに `premium` が有効で並んでいれば、アプリは正しく動いていることになる
（直すのは付与の側）。**並んでいないのにアプリが使い放題として振る舞うなら、
そのときは実装の問題**なので、`purchase_event_handler.dart` と
`ai_usage_repository.dart` の `is_unlimited` の書き込みを追う。

**上の「1. Transfer behavior」と同じ根**。1つのストアアカウントの権利が、
どの wanote アカウントにも渡ってしまう構造そのもの。

## 🔁 途中で止まっているもの（2026-08-23 夕方）

**Android の購入テストが再開できていない。**

`app-release.aab`（**versionCode 6**、`build/app/outputs/bundle/release/`）を
ビルド済み。リリースノートは日英とも用意済み
（`build/store_assets/release_notes_v6.md`。`build/` は gitignore なので
リポジトリには入っていない）。

| 状態 | |
|---|---|
| AAB のビルド | ✅ versionCode 6 |
| リリースノート（日英） | ✅ 用意済み |
| 内部テストへのアップロード | ❓ PM が実施。リリースは「公開中」 |
| 端末へのインストール | 🔴 **「エラーが発生しました」で入らない** |

**端末にアプリは入っていない状態**（`pm path` が空）。

### 分かっていること

- Play のログでは、アカウントもパッケージも正しく解決されている
  （`Selecting account [...] for package jp.wanote.app` /
  `GMS-INS: Metadata response for package:jp.wanote.app, status:4`）
- テスター参加リンクで **PM が誤って「テストを終了する」を押した**。
  つまりその前は参加済みだった → リンクも同意も正しかった
- 残る筋は**配信の反映待ち**。参加し直して数分〜数十分置く

### 再開したらやること

1. 参加リンクを開き直して「テスターになる」
2. 5〜10分置いてから Play ストア経由でインストール
3. だめなら Play ストアアプリのキャッシュを削除
4. 入ったら**チケット購入**を試す（今回の本命）

### 🔴 忘れないこと

**`adb install` で入れたビルドでは購入できない。** 商品一覧も金額も正しく出る
のに、購入した瞬間だけ `ITEM_UNAVAILABLE` が返る。同じ壁に2回当たっている。
詳細と使い分けは `docs/ANDROID_DEVICE_TEST_GUIDE.md` の冒頭。

## 待つだけのもの

| | 状態 |
|---|---|
| **Google の米国税務情報** | 🟡 審査中（2026-08-23 提出）。承認されれば B-1 完了 |

## 残っている作業

| # | 作業 | いつ |
|---|---|---|
| 0 | **iOS の輸出コンプライアンス情報** | 優先度：高。**次に iOS ビルドを上げるとき必ずぶつかる**。`BACKLOG.md` に詳細 |
| 1 | **スクリーンショットの撮り直し** | 優先度：高。`BACKLOG.md` に詳細 |
| 2 | **審査用スクリーンショット**（4商品） | 審査提出に必須。**これだけが足りていない** |
| 3 | Play のアプリ本体の配信地域 | **クローズドテスト完了後**（製品版トラックがロック中） |
| 4 | 審査用アカウントの作成 | 本番審査までに |
| 5 | プレミアムアクセス申告 | RevenueCat 設定後 |
| 6 | AdMob の本番ID | **クローズドテスト後**（下の「公開前の関門」参照） |

## 2026-08-23 に完了したこと

**手数料（両ストアとも 15% になった）**

- **Apple Small Business Program** ✅ 登録完了
- **Google 15% サービス手数料** ✅ 登録完了。
  🔴 **登録制だった。** 2021年に自動適用になったという理解は誤りで、
  Play Console のお知らせから登録が必要（受け付け開始 2026-08-18）。
  **適用は登録日からで遡らない。** 気づかなければ 30% で公開していた

**iOS の設定漏れ（4件、すべて未設定だった）**

| 項目 | 内容 |
|---|---|
| アプリ本体の価格 | 未設定 → **$0.00（無料）** を設定。無料アプリでも明示が要る |
| アプリ本体の配信地域 | 未設定 → **日本・米国のみ** |
| プレミアム月額の販売地域 | 未設定 → **日本・米国のみ** |
| プレミアム年額の販売地域 | **全175か国のまま** → 日本・米国のみ |

年額は気づかなければ EU に配信され、**DSA のトレーダー要件で個人の住所が
ストア上に公開される**ところだった。

**その他**

- サブスクリプショングループのローカリゼーション（日英）✅ 登録
- 確認して問題なかったもの: iOS 4商品のローカリゼーション（日英とも登録済み）、
  日本価格 ￥480（収益 ￥408）、Play の商品4つの販売地域（2か国・有効）、
  Android デベロッパー確認（要件を満たしている）

## ブラウザ連携について

**Claude in Chrome を接続済み。** ストアのコンソールを直接見られるので、
設定漏れの洗い出しに有効だった（この日の5件はすべてそれで見つかった）。
**重いのはスクリーンショットで、ブラウザ連携そのものの消費は思ったほどでは
なかった**（PM, 2026-08-23）。ページのテキストや要素を読む操作（get_page_text /
read_page / find）は軽い。**画面を撮るのは、そうしないと分からないときだけに
する。**

**保存・送信は必ず PM が押す**運用で進めた。これは維持する。

## 🔴 作業の進め方（PM指示）

**push は指示があったときだけ。** push のたびに Codemagic が動き、
ビルド分は有料。複数の変更をまとめてから push する。
**iOS ビルドも私からは起動しない**（有料のため PM が手動起動）。

現在のブランチ: **`main`**

## 🔴 公開前の関門: AdMob の本番IDに差し替える

**IDは 2026-08-23 に取得済み。** 差し替えがまだ。いまビルドに入っているのは
Google のテストIDで、**そのまま公開すると広告は出るのに収益が1円も発生しない。**

### 取得済みのID（AdMob アカウント: Play と同じ Google アカウント）

| 用途 | ID |
|---|---|
| アプリID Android | `ca-app-pub-1066694666239719~6724082179` |
| アプリID iOS | `ca-app-pub-1066694666239719~3658935134` |
| バナー Android | `ca-app-pub-1066694666239719/2386371598` |
| バナー iOS | `ca-app-pub-1066694666239719/9881718236` |
| インタースティシャル Android | `ca-app-pub-1066694666239719/2074390720` |
| インタースティシャル iOS | `ca-app-pub-1066694666239719/9897040420` |

### 差し替え先

| ID | 入れる場所 |
|---|---|
| **アプリID Android** | `android/app/src/main/AndroidManifest.xml`（`com.google.android.gms.ads.APPLICATION_ID`）**直書き** |
| **アプリID iOS** | `ios/Runner/Info.plist`（`GADApplicationIdentifier`）**直書き** |
| 残り4つ | `config/prod.json`（`ADMOB_BANNER_*` / `ADMOB_INTERSTITIAL_*`） |

アプリIDだけビルド時変数で渡せないため直書きになる。

### 🔴 いま差し替えてはいけない

**未公開アプリには本番広告がほぼ配信されない。** AdMob 側にも
「お支払い情報が設定されていないため、広告ユニットが広告を掲載していません」
「広告配信を制限しています」と出ている。いま切り替えると、テスターの端末で
広告が出なくなり、**不具合報告のように見える。**

**テストIDのままの方が広告まわりを確認できる。** 差し替えはクローズドテストの
後、公開の直前。
...
| 作業 | 状態 |
|---|---|
| **お支払いプロファイル** | ✅ 完了（2026-08-23）。アプリの審査が開始される |
| ストア情報の追加（両アプリ） | 🔴 未。公開後にストアと紐づける。トラフィック上限が上がる |
| アプリの審査・承認 | お支払いプロファイル待ち |
...
| ストア | 状態 |
|---|---|
| **Play サブスク** | ✅ 月額 ¥480/$4.99、年額 ¥4,800/$39.99 で登録完了 |
| **Play チケット** | 🔴 未登録（アイコン必須と判明。作成済み） |
| **Apple サブスク** | ✅ 登録完了（提出準備中） |
| **Apple チケット** | ✅ 登録完了（提出準備中） |
| **RevenueCat** | ✅ アカウント作成、商品取り込み、Entitlement、Offering まで完了 |
| **公開キー** | ✅ `config/prod.json` と Codemagic の両方に設定済み |

**金額の決定と根拠は `wanote/docs/PRICING.md` が正。**

## 見つけて直したバグ

**両ストアで商品IDの形が違う**ことが、RevenueCat への取り込みで判明した。
...
生の文字列で比較していたため、**プラン画面のラベル切り替えが iOS でだけ当たり、
Android では外れて**いた。`ProductIds.baseId` で `:` の前だけを見るようにした。
**消耗型にも同じ正規化を通した**（将来 Play が接尾辞を付け始めたら、チケットの
付与が黙って壊れるため）。

## CI のビルド分を使い切った

**原因は Claude の頻繁な push。** `verify` が push のたびに走る設定で、
その大半が `docs/` の変更だけだった。`codemagic.yaml` に changeset の除外と
`cancel_previous_builds` を入れた。**PM が課金を有効にして解消。**

## 手順書に反映したもの（実画面と食い違っていた箇所）

| 箇所 | 何が違ったか |
|---|---|
| G-2（Play アプリ内アイテム） | **アイコンが必須**。記載が無かった |
| G-2 | **購入オプション**の階層が増えていた |
| A-1（Apple サブスク） | **保存ボタンが無い**（自動保存） |
| A-2（Apple App内課金） | **こちらは手動保存**。サブスクと挙動が違う |
| R-0 | アカウント登録の手順が無かった。**英語の対訳表**を追加 |
| R-1b | **`.p8` が3種類**あり用途が違う。使い回してはいけない |
| A-4 | Sandbox テスターと wanote のアカウントは**別物** |

---

# 保留にしたもの（`wanote/docs/BACKLOG.md`）

| 項目 | 優先度 |
|---|---|
| 配信地域を日本・米国に絞る（**EU は DSA で住所が公開される**） | 公開前 |
| リアルタイム通知（Apple・Android とも未設定） | 公開前 |
| Google の手数料が登録制か（**10% と 30% で3倍差**） | B-1 後 |
| Financial reports bucket ID | B-1 後 |
| 写真からの BCS 判定 | 公開後 |
| Xcode Cloud への移行（**年会費に25時間込み**） | 公開後 |
...
`C:\Dev\wanote\build\store_assets\screenshots\`

`home` / `weight` / `toilet` / `prevention` / `medication_list` /
`medication_form` / `food` — **すべて 1064x2128（比率ちょうど 2.000）**。

**おすすめの並び順**: home → weight → toilet → prevention → food。
「何のアプリか → 記録 → 管理 → 便利機能」と読める。

**給餌AIの画面は入れていない。** Markdown 記法が生で出るバグがあるため（後述）。

### 加工の設定（再撮影するとき用）

**端末そのままの 1080x2280 は Play に弾かれる。**
規定は「最大辺を最小辺の2倍以上にできない」で、2280/1080 = 2.11。

この端末は**画面の縁に緑色の枠を 4px 描くことがある**（原因未特定。
アプリではなく端末側。8/19 夜の撮影では出ていなかった）。

```python
BOX = (8, 96, 1072, 2224)   # -> 1064x2128、比率ちょうど 2.000
```

## デモ用アカウント

PM が新規作成した。**旧アカウントは「変な写真ばかり」で掲載に使えない**ため。
パスワードは私（Claude）は知らないし、知ってはならない。

**注意: アプリを `force-stop` してはいけない。** セッションが切れて
再ログインが必要になり、PM の手を止める（実際にやった）。

| 対象 | 内容 |
|---|---|
| ペット | モカ / ミニチュア・ダックスフンド / 2021-05-20 / 4.8kg / 首28・胴42・着丈33 |
| 体重 | 2026/5/24〜8/17 の **9件**（5.00→4.80kg） |
| トイレ | 8/17〜8/19 の **8件** |
| 健康 | 1件（食欲不振） |
| 予防医療 | 既定4プログラム＋狂犬病に接種記録1件（2026-04-10 / 次回 2027-04-10） |
| 投薬 | ネクスガート スペクトラ（PM が登録） |

## コード修正（この日の分）

**すべて `flutter analyze` 通過済み。テストも通っている。**

| ファイル | 内容 |
|---|---|
| `health_record_form_screen.dart` | ①記録日時が `toString()` のままで `2026-08-19 00:51:56.861644` と生の値が出ていた → `formatDateTime` に変更 ②日付を選び直すと時刻が 00:00 に消えていた → 時分を保持 |
| `home_screen.dart` | **「餌の量」ショートカットが反応しないことがある**バグ。`_openFoodPortion` が Firestore の読み取りを**無制限に await** していた（タイムアウトも待機表示も例外処理もなし）。3秒でタイムアウトし、失敗してもプロフィールの体重で必ず画面を開くよう修正 |
| `wanote_loading_indicator.dart` | ローディングの文字が離れて見える件。原因は画像の余白ではなく**脈動アニメーション**（`Transform.scale` が中心基準で 0.92〜1.00 に拡縮 → 縮んだ瞬間に画像の下端が浮き、間隔が 4px〜約12px で揺れる）。基準点を**下端に固定**し、余白を 4px → 2px |

## adb 操作で踏んだ落とし穴

| 事象 | 対処 |
|---|---|
| `input text` は **ASCII しか送れない** | かな漢字は PM に入力してもらう。`cmd clipboard` はこの端末では未実装 |
| `KEYCODE_BACK` でアプリごと背面に落ちた | 使わない。キーボードは `完了` キーをタップして閉じる |
| 時刻入力で **2桁の「時」がずれる**（12:40 → 01:40） | 分の欄をタップ後 `KEYCODE_MOVE_END` → `DEL`×3 → 入力 |
| 画面消灯 30 秒ですぐロックされる | PM の許可を取って `screen_off_timeout` を延長する。**終わったら必ず戻す** |

**端末の設定は毎回すべて元に戻している**（ナイトモード `yes` /
画面消灯 `30000` ms / Wi-Fi・モバイルデータ ON / `wm` は未変更）。

---

# バックログに登録したもの（`docs/BACKLOG.md`）

## 1. AI の回答が Markdown のまま表示される 🔴

**AI 機能全体に影響する。** 実際に出た文字列:
...
| 画面 | 箇所 |
|---|---|
| 給餌アドバイス | `food_portion_screen.dart:555` — 素の `Text` |
| AI相談 | `consultation_screen.dart:357` — 素の `SelectableText` |

プロンプト側にも「Markdown を使うな」の指示がない。
**推奨は「プロンプト指示＋表示前の記法除去」を先に入れる**（依存追加なしで済む）。
Markdown として描画する案は `flutter_markdown` が2025に開発終了しており、
代替パッケージの選定が要る。

## 2. ホームのショートカットが反応しないことがある（原因未特定）

**再現には成功している。** ホーム→ショートカットを4種×8ラウンド=32回自動実行し、
**6回失敗**（画面がホームのまま。ベースラインとの差分 0.0 = 完全一致）。

**失敗はラウンド1〜2に集中し、3〜8では0件。**
アプリを前面に出してから**最初の30〜40秒だけ**起きている。
4種類すべてで起きるので、修正済みの「餌の量」固有の問題とは**別**。

**当初リマインダー通知との関連を疑ったが、PM より因果関係は無いと確認済み。**

次にやること: `flutter run` で繋いだ状態で同じ32回テストを流し、
`_openSectionTab` と `_applyTabRequest` に一時的な `developer.log` を入れて
失敗時のログを取る。

## 3. 予防医療の一覧に「次回予定日」が出ない

仕様変更にあたるため PM の判断待ち。

---

# メール（`support@wanote.jp`）の状態

**設定は正しい。反映待ちのみ。**

2026-08-19 夜、PM がネームサーバーを `ns-rs1/ns-rs2.gmoserver.jp` に変更。
**この切り替え先には正しいゾーンがある**（前回の `dns01/dns02.gmoserver.jp` は
空で REFUSED だった。今回は違う）。

```
MX  : mail21.onamae.ne.jp / 優先度10          ✅
TXT : v=spf1 include:_spf.onamae.ne.jp ~all   ✅
```

**残るは .jp レジストリへの反映のみ**（数時間〜24時間）。
**ネームサーバーをこれ以上触らないこと。**

反映後、**外部（Gmail等）から `support@wanote.jp` 宛にテスト送信**して確認する。
Webメールからの自送信は同一サーバー内で完結し DNS を見ないため**確認にならない**
（実際に一度これで誤判定しかけた）。
...
| # | 作業 | 備考 |
|---|---|---|
| 1 | **お支払いプロファイル** | **課金まわり全部の先頭**。個人／法人の判断が先。銀行口座・税務情報が要る。審査に数日 |
| 2 | Apple の商品登録（第2部 A-1〜A-3） | Google と独立。待ち時間にできる |
| 3 | Codemagic で iOS 再ビルド | ATT 対応は push 済みだが TestFlight ビルドに入っていない |
| 4 | テスター募集の開始 | 投稿文とフォーム設計は `TESTER_RECRUITMENT.md` に用意済み |

---

## いまの状況（2026-08-18 夕）

### 直近のコミット（push 済み・iOS再ビルド実施中）
- `95b09e2` 広告バックドロップのロック／画面残像／PDFプレビュー／ローディング
- `9047fb2` 予防医療リマインダーのタイミング設定（当日〜14日前・複数可）＋注意書き
- `95b1c2f` タブのスワイプ／体重の修正削除／トイレの＋ボタン／キーボード閉じる
- `4e0cc2e` Appleログイン(Android)・Googleログイン(iOS)・行削除の統一
- `488fddf` R5 確認リスト

### 原因が判明した主なもの（再発防止のため記録）
- **広告で操作不能** … 黒い全面レイヤーがタッチを吸収したまま残る。
  SDKコールバック待ちで最大2分。→ 3秒の監視タイマーで自力解除
- **画面の残像** … 押された画面が `backgroundColor: Colors.transparent`。
  スライド中に下の画面が透ける。→ `PatternedBackground` で不透明化
- **PDFプレビューが出ない** … `Printing.layoutPdf` の `onLayout` の中で
  フォントをネットワーク取得。例外がプラグイン内で握り潰される。
  → プレビューを開く前にバイト列を組み立てる
- **Appleログイン(Android)** … `webAuthenticationOptions` 未指定でWebフローが
  開始できない。iOSはOSに直接聞くので影響なし
- **Googleログイン(iOS)** … CIが差し替える `GoogleService-Info.plist` と、
  リポジトリ内の `Info.plist` のURLスキームがずれる。
  → ビルド時に `REVERSED_CLIENT_ID` から生成（**仮説であり要検証**）

### PMにお願いしたいこと
- iOSビルド後、R5 の **C-1・C-2・C-3** を確認
- ペットのサイズの**図解画像の差し替え**（用意でき次第 `C:\Dev\docs\` へ）

### 残っていること
- **iOS権限ダイアログの英語化** … `wanote/docs/BACKLOG.md`。pbxproj編集を伴う
...
```
C:\Dev\docs\SESSION_HANDOVER.md を読んで、続きから作業を再開してください。
```
...
`C:\Dev\docs\PRIVACY_POLICY_DRAFT.md` に日本語の下書きを作成。
...
### メッセージ一覧Excel（`C:\Dev\docs\wanote_messages.xlsx`）とARBへの反映
...
> `C:\Dev\docs\tools\verify_arb_against_excel.py` を `py -3` で実行。
...
**成果物**: `C:\Dev\docs\wanote_messages.xlsx` の **「画面一覧」シート**に、
20画面 × 日英 = **40枚**を並べて埋め込み済み（差分ゼロ）。
PNG実体は `C:\Dev\docs\screenshots\`（751×1624・実フォント）。
...
再現手順とツールキットは `C:\Dev\docs\tools\tour_toolkit.js` に保存。
...
| スクリーンショット受信 :5050 | 起動したまま | 落ちていたら `py -3 C:\Dev\docs\tools\shot_receiver.py` |
...
| パス | 内容 |
|---|---|
| `C:\Dev\docs\wanote_messages.xlsx` | メッセージ一覧（G/H列記入済み・最新） |
| `C:\Dev\docs\tools\fix_excel.py` | Excel G/H列生成スクリプト（全訳文を保持） |
| `C:\Dev\docs\tools\fix_arb.py` | app_ja.arb への翻訳適用スクリプト |
| `C:\Dev\docs\tools\build_messages_excel.py` | ARBからExcelを生成 |
| `C:\Dev\docs\tools\shot_receiver.py` | スクリーンショット受信サーバ(:5050) |
| `C:\Dev\docs\screenshots\` | 取得済みスクリーンショット2枚 |
